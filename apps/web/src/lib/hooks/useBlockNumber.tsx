...
export function BlockNumberProvider({ children }: PropsWithChildren) {
    ...
    setChainBlock((chainBlock) => {
      if (chainBlock.chainId === chainId) {
        if (!chainBlock.block || chainBlock.block < block) {
          const mainnetBlock = chainId === UniverseChainId.Zanjir ? block : chainBlock.mainnetBlock
          return { chainId, block, mainnetBlock }
        }
      } else if (chainId === UniverseChainId.Zanjir) {
  ...
  useEffect(() => {
    RPC_PROVIDERS[UniverseChainId.Zanjir]
      .getBlockNumber()
      .then((block) => onChainBlock(UniverseChainId.Zanjir, block))
